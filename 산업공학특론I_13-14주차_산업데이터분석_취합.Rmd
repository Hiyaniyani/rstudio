...
# 모델 수립
trainsc1 <- as.data.frame(trainsc1)
trainsc2 <- as.data.frame(trainsc2)

pcr_model1 <- pcr(Stage1_Output~., data=trainsc1, validation="CV")
summary(pcr_model1) # 19개 주성분이 최적

pcr_model2 <- pcr(Stage2_Output~., data=trainsc2, validation="CV")
summary(pcr_model2) # 22개 주성분이 최적

plot(RMSEP(pcr_model1), legendpos="topright")
plot(pcr_model1, ncomp=19, line=T)
plot(pcr_model1, "loadings", comps=1:19, legendpos="topright")

plot(RMSEP(pcr_model2), legendpos="topright")
plot(pcr_model2, ncomp=22, line=T)
plot(pcr_model2, "loadings", comps=1:22, legendpos="topright")
...
pred_pcr1 <- predict(pcr_model1, testsc1, ncomp=19)
err(testsc1$Stage1_Output, pred_pcr1)

pred_pcr2 <- predict(pcr_model2, testsc2, ncomp=22)
